...
import "../../../../Helpers/PinyinHelper.js" as PinyinHelper
  ...
  function loadApplications() {
    ...
    entries = allApps.filter(app => {
                               ...
                               return true;
                                       ...
                                       app._namePinyin = PinyinHelper.toPinyinCompact(app.name || "").toLowerCase();
                                       app._namePinyinInitials = PinyinHelper.toPinyinInitials(app.name || "").toLowerCase();
                                       return app;
                                     });
  ...
  function getResults(query) {
    ...
    if (typeof FuzzySort !== 'undefined') {
      const fuzzyResults = FuzzySort.go(query, filteredEntries, {
                                          "keys": ["name", "comment", "genericName", "executableName", "_namePinyin", "_namePinyinInitials"],
                                          "limit": 20
                                        });
      ...
      return filteredEntries.filter(app => {
                                      ...
                                      const namePy = app._namePinyin || "";
                                      const nameInit = app._namePinyinInitials || "";
                                      return name.includes(searchTerm) || comment.includes(searchTerm) || generic.includes(searchTerm) || executable.includes(searchTerm) || namePy.includes(searchTerm) || nameInit.includes(searchTerm);
                                    }).sort((a, b) => {
